...
def load_smplx(template='assets/models/SMPLX_{}.fbx',
              gender='female'):
    """
    Loads smpl model, deleted armature and renames mesh to 'Body'
    """
...
def randomized_verts(model,
   ...
   # randpose[:body_idx] = pose_data[randframe]
    ...
    randpose[:3]=[0,0,0]
    # randpose[0]+=np.pi
    # randpose[3:body_idx-6]=body_pose.detach().numpy()[0]
    # randpose[body_idx-6:body_idx]=[0,0,0,0,0,0]


    # Overwrite global rotation with uniform random rotation
    # global_orient[0]+=np.pi
    # randpose[0:3]=global_orient
    # randpose[0]=randpose[0]+np.pi
    # randpose[0:3]=[0,0,0]
...
def randomized_verts_smplx(model,
    ...
    # Init with zero trans
    # model.trans[:] = np.zeros(model.trans.size)
    new_model['transl'] = np.zeros(3)

    # Set random shape param
    # randshape = betas
    randshape = betas.detach().cpu().numpy()
    # model.betas[:] = randshape
    new_model['betas'] = randshape

    # Random body pose (except hand)
    # randpose = np.zeros(model.pose.size)
    randpose = np.zeros(63)
    # body_idx = 72
    # randpose[:body_idx] = body_pose
    # randpose[:3] = [0, 0, 0]
    ...
    # translation = torch.from_numpy(trans).unsqueeze(dim=1)
    use_cuda = False
    ...
    output = model(betas=betas, expression=expression, # transl=translation,
                   body_pose=body_pose,
                   jaw_pose=torch.from_numpy(jaw_pose).to(device=device), use_pca=True, num_pca_comps=12,
                   # left_hand_pose=torch.from_numpy(hand_pose_l), right_hand_pose=torch.from_numpy(hand_pose_r),
                   gender='female', return_verts=True)
    new_verts = output.vertices.detach().cpu().numpy().squeeze()
    # new_verts -= [0, 0, 0.5]
    new_verts -= trans
