...
def create_segmentation_smplx(ob, path_assets='assets'):
    import bpy
    print('creating segmentation')
    materials = {}
    vgroups = {}
    segm_file = os.path.join(path_assets, 'segms', 'smplx_vert_segmentation.json')
    ...
    parts = sorted(vsegm.keys())
    sorted_parts = [
        'spine2', 'leftUpLeg', 'hips', 'spine', 'leftLeg', 'rightLeg',
        'neck', 'spine1', 'head', 'leftArm',
        'rightArm', 'leftForeArm', 'rightForeArm',
        'leftShoulder', 'rightShoulder', 'rightUpLeg',
        'leftEye', 'rightEye', 'leftHand','rightHand',
        'leftHandIndex1', 'rightHandIndex1',
        'leftToeBase', 'rightToeBase',
        'leftFoot', 'rightFoot', 'eyeballs'
    ]
    part2num = {part: (ipart + 1) for ipart, part in enumerate(sorted_parts)}
    # part_body = 'neck'
    # for body_part in part2num:
    #     if body_part != part_body:
    #         part2num[body_part] = 1
    # part2num[part_body] = 16

    for part in parts:
        vs = vsegm[part]
        vgroups[part] = ob.vertex_groups.new(name=part)
        vgroups[part].add(vs, 1.0, 'ADD')
        bpy.ops.object.vertex_group_set_active(group=part)
...
def initialize_texture(obj, path_assets='assets', texture_zoom=1, tmp_suffix=''):
    import bpy
    ...
    sh_dst = os.path.join(path_assets, 'lights', 'sh.osl')

    # Make temporary copy of osl file
    tmp_folder = os.path.join(path_assets, 'tmp_{}/osl'.format(tmp_suffix))
    os.makedirs(tmp_folder, exist_ok=True)
    filename = str(np.random.randint(0, 1000000)) + '.osl'
    while os.path.exists(os.path.join(tmp_folder, filename)):
        filename = str(np.random.randint(0, 1000000)) + '.osl'
    ...
    materials = create_segmentation_smplx(obj, path_assets=path_assets)
